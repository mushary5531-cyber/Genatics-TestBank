**Q1.** A lentiviral vector gene therapy product is used in the treatment of which of the following conditions?

A) Severe beta-thalassemia / hemoglobinopathy
...
A) Modifying an existing gene's activity (splicing modulation)
B) Gene knockdown
C) Gene editing
...
B) They are found exclusively in reproductive tissue
C) They can differentiate into many different tissue types
...
leukemia-like disease. What is the most likely cause?

A) Insertion of the retroviral vector near a proto-oncogene promoter
B) The SCID disease itself causing cancer
C) Triggered HTLV infection
...
cells. However, their use is mainly hindered by:

A) They can only be used for ex-vivo gene therapy
B) They infect a wide spectrum of cells
C) Insertional mutagenesis (integration near oncogenes)
...
A) Requirement for co-infection with a helper virus
B) Requirement of cell division for infection
C) Unstable (transient) expression of the introduced gene
...
mRNA into host cells using which delivery system?
...
large-capacity vectors in gene therapy?

A) Ability to carry large DNA inserts (high cloning capacity)
B) High immunogenicity ensuring strong immune activation
C) Permanent stable genomic integration in all tissues
...
into cells. What is the mechanism by which it facilitates gene entry?

A) Creates permanent pores in the cell membrane
B) Uses viral capsid proteins to penetrate the membrane
C) Depends on receptor-mediated endocytosis
...
disorders such as Parkinson's disease because it transduces non-dividing
neurons without genomic integration?
...
promoting gene. Which approach directly blocks gene expression by binding
complementary mRNA sequences?

A) Gene replacement therapy (adding a functional gene copy)
B) Antisense therapy (antisense oligonucleotides)
C) Gene editing with CRISPR-Cas9
...
A) Tumor suppressor gene replacement (restoring p53 function)
B) Suicide gene therapy (HSV-TK + ganciclovir prodrug activation)
C) Immune gene therapy (CAR-T cell modification)
...
A) Embryonic stem cells derived from the inner cell mass of a blastocyst
B) Adult stem cells with limited differentiation potential
C) Mature (differentiated) somatic cells reprogrammed to pluripotency without using embryos
...
A) The cells have lost DNA sequences needed for pluripotency during differentiation
B) Genes required for pluripotency are permanently turned off by epigenetic silencing
C) Differentiated cells have a fundamentally different genetic code
